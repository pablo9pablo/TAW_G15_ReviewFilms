...
public interface ReviewRepository extends JpaRepository<Review,Integer> {
    ...
    @Query("SELECT r FROM Review r WHERE r.user = :user AND (:reviewDate IS NULL OR (r.date >= :reviewDate AND r.date <= CURRENT_DATE)) AND (:minRating IS NULL OR r.rating >= :minRating)")
    public List<Review> filterReviews(@Param("user") User user, @Param("reviewDate") LocalDate reviewDate, @Param("minRating") Integer minRating);
